...
const Requests = () => {
    ...
    useEffect(() => {
        const fetchPendingRequests = async () => {
            try {
                const response = await api.get('/encargado/solicitudes');
    ...
    return (
        <div className="max-w-7xl mx-auto pb-20 animate-fade-in font-sans">
            <h1 className="text-4xl font-display font-bold text-gray-900 mb-6">Solicitudes Pendientes</h1>
            ...
            {events.length === 0 ? (
                <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm text-center flex flex-col items-center">
                    ...
                    <p className="text-gray-500 font-medium">No hay solicitudes pendientes en tu departamento.</p>
                </div>
            ) : (
